...
function safeWriteHead (reply, statusCode) {
  ...
  try {
    if (headers['set-cookie'] !== undefined && res.hasHeader('set-cookie')) {
      // set-cookie headers written directly to the raw response
      // (e.g. by middleware) must be preserved, not overwritten
      res.writeHead(statusCode, { ...headers, 'set-cookie': mergeSetCookieHeader(res, headers['set-cookie']) })
    } else {
...
// Merges a set-cookie value from the reply headers with any set-cookie
// header already present on the raw response, so cookies written through
// either channel are all sent to the client.
function mergeSetCookieHeader (res, value) {
  const rawValue = res.getHeader('set-cookie')
  if (rawValue === undefined) {
    return value
  }
  const merged = Array.isArray(rawValue) ? rawValue.slice() : [rawValue]
  if (Array.isArray(value)) {
    merged.push(...value)
  } else {
    merged.push(value)
...
function writeReplyHeaders (res, reply) {
  const headers = reply[kReplyHeaders]
  for (const key in headers) {
    if (key === 'set-cookie') {
      res.setHeader(key, mergeSetCookieHeader(res, headers[key]))
    } else {
